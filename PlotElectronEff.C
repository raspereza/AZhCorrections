...
void PlotElectronEff(TString era = "2018",
  ...
  map<TString, TString> etaBinMap = {
    {"EtaLt1p0","|#eta|<1.0"},
    {"Eta1p0to1p48","1.0<|#eta|<1.48"},
    {"Eta1p48to1p65","1.48<|#eta|<1.65"},
    {"Eta1p65to2p1","1.65<|#eta|<2.1"},
    {"EtaGt2p1","2.1<|#eta|<2.5"}
  };
  

  TString name("Id/Iso");
  TString FileName = "ElectronID/UL"+era+"/Electron_RunUL"+era+"_IdIso_AZh_IsoLt0p15_IdFall17MVA80noIsov2.root";
  if (trigger) {
    name = "Ele35";
    FileName = "ElectronTrigger/UL"+era+"/Electron_RunUL"+era+"_Ele35.root";
    if (era.Contains("2016")) {
      FileName = "ElectronTrigger/UL"+era+"/Electron_RunUL"+era+"_Ele25_EtaLt2p1.root";
      name = "Ele25";
    }
  }
  ...
  SF->GetXaxis()->SetTitle("electron p_{T} [GeV]");
  SF->SetMarkerColor(1);
  ...
  canv->Update();
  if (trigger)
    canv->Print("Electron_trigger_"+EtaBin+"_"+era+".png");
  else
    canv->Print("Electron_ID_"+EtaBin+"_"+era+".png");
